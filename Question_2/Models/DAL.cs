...
namespace carterAPI.Models
{
    public class DAL
    {
        ...
        {
            Response resp = new Response();
            SqlDataAdapter da = new SqlDataAdapter("SELECT p.*, ci.*, c.* " +
                "FROM tbl_purchases p " +
                "LEFT JOIN tbl_customer_info ci ON ci.cust_id = p.cust_id " +
                "INNER JOIN tbl_car_info c ON c.car_id = p.car_id", con);
            DataTable dt = new DataTable();
            List<purchasesEndpoint> point = new List<purchasesEndpoint>();
            da.Fill(dt);
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    purchasesEndpoint list_point = new purchasesEndpoint();
                    carEndpoint list_cpoint = new carEndpoint();
                    Endpoint list_purchasepoint = new Endpoint();
                    list_point.purchase_id = Convert.ToInt32(dt.Rows[i]["id"]);
                    list_point.car_id = Convert.ToInt32(dt.Rows[i]["car_id"]);
                    list_point.cust_id = Convert.ToInt32(dt.Rows[i]["cust_id"]);
                    list_purchasepoint.cust_id = Convert.ToInt32(dt.Rows[0]["cust_id"]);
                    list_purchasepoint.first_name = Convert.ToString(dt.Rows[0]["first_name"]);
                    list_purchasepoint.last_name = Convert.ToString(dt.Rows[0]["last_name"]);
                    list_purchasepoint.phone_number = Convert.ToString(dt.Rows[0]["phone_number"]);
                    list_cpoint.car_id = Convert.ToInt32(dt.Rows[0]["car_id"]);
                    list_cpoint.car_name = Convert.ToString(dt.Rows[0]["car_name"]);
                    list_cpoint.car_model = Convert.ToString(dt.Rows[0]["car_model"]);
                    list_cpoint.car_image = Convert.ToString(dt.Rows[0]["car_image"]);
                    list_cpoint.year_released = Convert.ToInt32(dt.Rows[0]["year_released"]);
                    list_cpoint.no_of_liters = Convert.ToDecimal(dt.Rows[0]["no_of_liters"]);
                    list_cpoint.transmission = Convert.ToString(dt.Rows[0]["transmission"]);
                    list_cpoint.price = Convert.ToDecimal(dt.Rows[0]["price"]);
                    list_cpoint.monthly_installment = Convert.ToDecimal(dt.Rows[0]["monthly_installment"]);
                    list_cpoint.insurance = Convert.ToInt32(dt.Rows[0]["insurance"]);
                    list_cpoint.warrenty = Convert.ToInt32(dt.Rows[0]["warrenty"]);
                    list_cpoint.good_features = Convert.ToString(dt.Rows[0]["good_features"]);
                    list_cpoint.bad_features = Convert.ToString(dt.Rows[0]["bad_features"]);
        ...
        //public Response getPurchase(SqlConnection con)
        //{
        //    Response resp = new Response();
        //    SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM tbl_purchases", con);
        //    DataTable dt = new DataTable();
        //    List<purchasesEndpoint> point = new List<purchasesEndpoint>();
        //    da.Fill(dt);
        //    if (dt.Rows.Count > 0)
